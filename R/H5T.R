H5Tcreate <- function(type, size) {
  ...
  allowed_types <- c("H5T_STRING", "H5T_COMPOUND", "H5T_ENUM", "H5T_OPAQUE")
  if (!type %in% h5constants$H5T_CLASS[allowed_types]) {
    stop(
      "H5Tcreate can only be used with the following data types:\n",
...
H5Tset_strpad <- function(dtype_id, strpad = "NULLPAD") {
  strpad_int <- switch(
    ...
    SPACEPAD = 2L,
    stop(
      "Invalid value to 'strpad' argument.\n",
...
H5Tset_cset <- function(dtype_id, cset = "ASCII") {
  cset_int <- switch(
    ...
    "UTF-8" = 1L,
    stop(
      "Invalid value to 'cset' argument.\n",
