...
test_that("Write by index and hyperslab works.", {
  ...
  expect_equal(
    h5read(h5File, "D")[c(1:3, 5:7), c(1:4, 6:9)],
    matrix(1:48, nrow = 6, ncol = 8)
  )
  res <- h5read(
    file = h5File,
...
test_that("Writing to file handle", {
  ...
  expect_silent(h5writeDataset(
    obj = matrix(1:10, ncol = 2),
    h5loc = fid,
    name = "matrix"
  ))
  ...
  expect_silent(h5writeDataset(
    obj = list(a = 1:4, b = letters[10:15]),
    h5loc = fid,
    name = "list"
  ))
  expect_silent(h5writeDataset(
    obj = data.frame("col_A" = 1:10, "col_B" = letters[1:10]),
    h5loc = fid,
    name = "data.frame"
  ))
  expect_silent(h5writeDataset(
  ...
  expect_silent(h5writeDataset(
    obj = complex(length.out = 10, real = 1:10, imaginary = 10:1),
    h5loc = fid,
    name = "complex"
  ))
  H5Fclose(fid)
...
test_that("Changing chunk size works", {
  expect_silent(h5write(
    data.frame("A" = rep(1:100), "B" = 201:300),
    h5f1,
    "dset",
    level = 7,
    chunk = 1L
...
test_that("We can write a data.frame with multiple factor columns", {
  ...
  expect_equivalent(h5read(file = h5f1, name = "data/Y"), as.character(Z$Y))
})
